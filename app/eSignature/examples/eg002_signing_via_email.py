...
class Eg002SigningViaEmailController:
    ...
    def make_envelope(cls, args, doc_docx_path, doc_pdf_path):
        ...
        # Create signHere fields (also known as tabs) on the documents,
        # Using anchor (autoPlace) positioning with "Please Sign Here"
        sign_here_please_sign = SignHere(
            anchor_string="Please sign below to accept your offer of employment. ",
            anchor_units="pixels",
            anchor_y_offset="10",  # Adjust as needed
            anchor_x_offset="20"   # Adjust as needed

        )


        # Add the tabs model (including the new sign_here tab) to the signer
        # signer1.tabs = Tabs(sign_here_tabs=[sign_here_please_sign])
        ...
        signer1.tabs = Tabs(sign_here_tabs=[sign_here1, sign_here2, sign_here_please_sign])
